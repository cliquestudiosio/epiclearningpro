...
- **Contact form: Resend over the Workers `fetch` API, no SMTP.** Workers can't hold raw SMTP sockets the way `nodemailer` needs, so the contact form calls Resend's HTTPS API directly instead. Requires `RESEND_API_KEY` set on the deployed Worker (Workers Builds → Settings → Variables and Secrets → **Runtime**, add it as a **Secret** so its value stays encrypted — not a plain Variable; plain Variables aren't declared in `wrangler.jsonc` and get wiped by every Workers Builds redeploy). `CONTACT_TO_EMAIL` / `CONTACT_FROM_EMAIL` are committed in `wrangler.jsonc`'s `vars` block instead of set via the dashboard — same reasoning (a per-deployment, non-secret value that Workers Builds would otherwise wipe), and they sit alongside the equally client-specific `routes` entry there. Update both when forking this for a new client. Since frontend and API are same-origin now, no CORS handling is needed (the old Express version needed the `cors` package for exactly this reason).
  - **`CONTACT_FROM_EMAIL`'s domain must exactly match a domain verified in Resend — a subdomain and its root are different domains to Resend.** This site's Resend account has `portfolio.cliquestudios.io` verified (a subdomain), not the root `cliquestudios.io`. So `CONTACT_FROM_EMAIL` has to be `<anything>@portfolio.cliquestudios.io` — `<anything>@cliquestudios.io` 403s ("Domain not verified") even though it looks like the same domain. This only applies to the `from` address; `CONTACT_TO_EMAIL` (the actual inbox, `portfolio@cliquestudios.io`) needs no verification at all — Resend only verifies the sending domain, never the recipient.
...
- **Page stays hidden until saved content is applied, to prevent a flash of default content.** Since Supabase content loads asynchronously after React's own initial paint, without this every visitor would briefly see the hardcoded defaults before their real edits appeared. `index.html`'s `<head>` sets `body{opacity:0}` inline and defines `window.__apReveal()` with a 900ms hard-cap safety net (so an unreachable/slow backend never leaves the page blank); `admin-portal.js`'s `boot()` calls it once content has actually been applied. The network fetch and the pre-existing 420ms settle delay (`DELAY` — originally there to let React finish its first render before the DOM overlay runs) execute concurrently, not back-to-back, to keep the hidden window as short as possible. Verified via headless Chromium that the page correctly stays hidden through the full safety-net window and reveals cleanly with no flash when the backend is unreachable (this sandbox can't reach `supabase.co` — see the Gotchas entry on that); the faster, backend-reachable path is architecturally the same mechanism, just revealing sooner.
